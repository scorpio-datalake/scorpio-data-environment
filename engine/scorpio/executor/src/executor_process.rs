...
use scorpio_core::config::{LogRotationPolicy, TaskSchedulingPolicy};
use scorpio_core::error::BallistaError;
use scorpio_core::extension::{EndpointOverrideFn, SessionConfigExt};
use scorpio_core::serde::protobuf::executor_resource::Resource;
use scorpio_core::serde::protobuf::executor_status::Status;
...
use scorpio_core::{BALLISTA_VERSION, ConfigProducer, RuntimeProducer};
...
pub async fn start_executor_process(
    ...
    info!("Scorpio executor v{BALLISTA_VERSION} (DataFusion v{DATAFUSION_VERSION}) starting ...");
            ...
            while !shuffle_cleaner_shutdown.is_shutdown() {
                tokio::select! {
                    _ = interval_time.tick() => {
                        ...
                        {
                            error!("Scorpio executor failed to clean_shuffle_data {e:?}")
                        }
                        },
...
async fn clean_shuffle_data_loop(work_dir: &str, seconds: u64) -> scorpio_core::error::Result<()> {
...
async fn clean_all_shuffle_data(work_dir: &str) -> scorpio_core::error::Result<()> {
    let mut dir = fs::read_dir(work_dir).await?;
    let mut to_deleted = Vec::new();
    while let Some(child) = dir.next_entry().await? {
        if let Ok(metadata) = child.metadata().await {
            ...
            override_arrow_flight_service: Some(std::sync::Arc::new(
                move |address, mut grpc_shutdown, ballista_config| {
                    tokio::spawn(async move {
                        ...
                        let server_future =
                            scorpio_core::utils::create_grpc_server(&ballista_config)
                                .add_service(
                        ...
                        server_future.await.map_err(|e| {
                            ...
                            scorpio_core::error::BallistaError::TonicError(e)
                        })
                    })
                },
            )),
