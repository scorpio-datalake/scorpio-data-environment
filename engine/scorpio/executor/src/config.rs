...
pub struct Config {
    ...
    #[arg(short = 's', long, default_value_t = scorpio_core::config::TaskSchedulingPolicy::default(), help = "The task scheduling policy used by scheduler. Configuration must match with scheduler configured policy.")]
    pub task_scheduling_policy: scorpio_core::config::TaskSchedulingPolicy,
    ...
    #[arg(
        long,
        default_value_t = scorpio_core::config::LogRotationPolicy::Daily,
        help = "Tracing log rotation policy."
    )]
    pub log_rotation_policy: scorpio_core::config::LogRotationPolicy,
...
impl TryFrom<Config> for ExecutorProcessConfig {
    ...
    fn try_from(opt: Config) -> Result<Self, Self::Error> {
        Ok(ExecutorProcessConfig {
            ...
            grpc_server_config: scorpio_core::utils::GrpcServerConfig::default(),
