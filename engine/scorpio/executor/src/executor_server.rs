...
use scorpio_core::serde::scheduler::TaskDefinition;

use scorpio_core::serde::scheduler::from_proto::{get_task_definition, get_task_definition_vec};
use scorpio_core::utils::{create_grpc_client_endpoint, create_grpc_server};
...
pub async fn startup<T: 'static + AsLogicalPlan, U: 'static + AsExecutionPlan>(
    ...
    let server = {
        ...
        info!("Scorpio executor gRPC server v{BALLISTA_VERSION} listening on {addr:?}");
...
impl<T: 'static + AsLogicalPlan, U: 'static + AsExecutionPlan> ExecutorServer<T, U> {
    ...
    async fn heartbeat(&self) {
            ...
            .await
        {
            ...
            Err(e) => {
                warn!("Fail to update heartbeat to its registration scheduler due to {e:?}");
            }
        };
        ...
        for mut item in self.schedulers.iter_mut() {
                ...
                .await
            {
                ...
                Err(e) => {
                    warn!("Fail to update heartbeat to scheduler {scheduler_id} due to {e:?}");
                }
            }
        }
    }
        ...
        match (exec, runtime) {
            (Ok(exec), Ok(runtime)) => {
                ...
                let execution_result = self
                    .executor
                    .execute_query_stage(task_id, part.clone(), exec.clone(), task_context)
                    .await;
                    ...
                    match executor_server.get_scheduler_client(&scheduler_id).await {
                        Ok(mut scheduler) => {
                            if let Err(e) = scheduler
                                ...
                                .await
                            {
                                error!("Fail to update tasks {tasks_status:?} due to {e:?}");
                            }
                        }
                        Err(e) => {
                            error!("Fail to connect to scheduler {scheduler_id} due to {e:?}");
                        }
                    }
                }
            }
