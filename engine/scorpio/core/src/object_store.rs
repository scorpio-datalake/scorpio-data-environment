...
//!
//! **Integration:** MinIO (S3-compatible) round-trip tests live in `scorpio/core/tests/s3_minio_integration.rs`
...
mod tests {
    ...
    #[test]
    fn url_host_is_azure_detects_fabric_endpoints() {
        for url in [
            "https://foo.dfs.fabric.microsoft.com/container",
            "https://bar.blob.fabric.microsoft.com/container",
        ] {
            let u = Url::parse(url).unwrap();
            assert!(
                CustomObjectStoreRegistry::url_host_is_azure_blob_or_dfs(&u),
                "expected azure host: {url}"
            );
        }
    }

    #[test]
    fn object_store_registry_key_gs_bucket_style() {
        let u = Url::parse("gs://my-bucket/prefix/obj").unwrap();
        assert_eq!(
            CustomObjectStoreRegistry::object_store_registry_key(&u),
            "gs://my-bucket"
        );
    ...
    #[test]
    fn object_store_registry_key_https_includes_port_when_present() {
    ...
    #[test]
    fn get_store_errors_for_unsupported_scheme_when_not_registered() {
        let reg = CustomObjectStoreRegistry::new(S3Options::default());
        let u = Url::parse("ftp://example.com/path").unwrap();
        assert!(
            reg.get_store(&u).is_err(),
            "unregistered non-file/http/s3/gs/azure schemes should error"
        );
    }

    #[test]
    fn object_store_registry_key_abfss_preserves_authority() {
        let u = Url::parse("abfss://mycontainer@acct.dfs.core.windows.net/p").unwrap();
        let k = CustomObjectStoreRegistry::object_store_registry_key(&u);
        assert!(
            k.starts_with("abfss://") && k.contains("acct.dfs.core.windows.net"),
            "unexpected registry key (DataFusion-aligned authority): {k}"
        );
    }

    #[test]
    fn object_store_registry_key_azure_scheme() {
        let u = Url::parse("azure://container/path/to/blob").unwrap();
        assert_eq!(
            CustomObjectStoreRegistry::object_store_registry_key(&u),
            "azure://container"
        );
