...
/// Client for interacting with remote executors.
#[derive(Clone)]
...
impl BallistaClient {
    /// Create a new client to connect to the executor listening on the specified
    /// host and port
    pub async fn try_new(
        ...
        debug!("engine client connecting to {addr}");

        let mut endpoint = create_grpc_client_endpoint(addr.clone(), None).map_err(|e| {
            BallistaError::GrpcConnectionError(format!(
                "Error creating endpoint to scheduler or executor at {addr}: {e:?}"
            ))
        })?;
        ...
        let connection = endpoint.connect().await.map_err(|e| {
            BallistaError::GrpcConnectionError(format!(
                "Error connecting to scheduler or executor at {addr}: {e:?}"
            ))
        })?;
        ...
        debug!("executor Flight client connected OK: {flight_client:?}");
