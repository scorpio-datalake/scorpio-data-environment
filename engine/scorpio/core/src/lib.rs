...
/// The current engine version string (Cargo package version of `scorpio-core`).
pub const BALLISTA_VERSION: &str = env!("CARGO_PKG_VERSION");

/// Prints the current engine version to stdout.
pub fn print_version() {
    println!("Scorpio engine (scorpio-core) version: {BALLISTA_VERSION}")
}

/// Client utilities for connecting to schedulers.
pub mod client;
/// Configuration options and settings for distributed engine components (`ballista.*` keys).
pub mod config;
/// Utilities for generating execution plan diagrams.
pub mod diagram;
/// Error types and result definitions for distributed engine operations.
pub mod error;
...
pub mod serde;
/// General utility functions for the distributed engine.
pub mod utils;
