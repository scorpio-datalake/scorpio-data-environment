...
pub struct Config {
    ...
    #[arg(short = 'n', long, default_value_t = String::from("ballista"), help = "Namespace for the cluster this scheduler coordinates (shared by executors that join it).")]
    ...
    #[arg(
        short = 's',
        long,
        default_value_t = scorpio_core::config::TaskSchedulingPolicy::default(),
        help = "The scheduling policy used by the scheduler. Executor configuration must match with scheduler configured policy."
    )]
    pub scheduler_policy: scorpio_core::config::TaskSchedulingPolicy,
...
impl TryFrom<Config> for SchedulerConfig {
    type Error = scorpio_core::error::BallistaError;
