...
use scorpio_core::utils::{GrpcClientConfig, create_grpc_client_endpoint};
...
fn from_ballista_err(e: &scorpio_core::error::BallistaError) -> Status {
    Status::internal(format!("Scorpio engine error: {e:?}"))
}
...
async fn get_flight_client(
    ...
    let mut endpoint =
        create_grpc_client_endpoint(addr.clone(), Some(&grpc_config)).map_err(|e| {
            BallistaError::GrpcConnectionError(format!(
                "Error creating endpoint for executor at {addr}: {e:?}"
            ))
        })?;
    ...
    if let Some(ref customize) = customize_endpoint {
        endpoint = customize.configure_endpoint(endpoint).map_err(|e| {
            BallistaError::GrpcConnectionError(format!(
                "Error customizing endpoint for executor at {addr}: {e}"
            ))
        })?;
    }
