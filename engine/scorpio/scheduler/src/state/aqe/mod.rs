...
impl AdaptiveExecutionGraph {
    ...
    pub fn try_new(
        ...
        logical_plan: Option<String>,
        physical_plan: Option<String>,
    ) -> scorpio_core::error::Result<Self> {
        ...
        let stages: scorpio_core::error::Result<HashMap<usize, ExecutionStage>> = runnable
            .into_iter()
...
impl AdaptiveExecutionGraph {
    ...
    ) -> scorpio_core::error::Result<(usize, ExecutionStage)> {
    ...
    ) -> scorpio_core::error::Result<Vec<QueryStageSchedulerEvent>> {
    ...
    ) -> scorpio_core::error::Result<HashSet<usize>> {
        self.planner
    ...
    fn reset_stages_internal(
        &mut self,
        executor_id: &str,
    ) -> scorpio_core::error::Result<(HashSet<usize>, Vec<RunningTaskInfo>)> {
...
impl ExecutionGraph for AdaptiveExecutionGraph {
    ...
    fn resolve_stage(&mut self, stage_id: usize) -> scorpio_core::error::Result<bool> {
    ...
    ) -> scorpio_core::error::Result<Vec<RunningTaskInfo>> {
        if let Some(ExecutionStage::Running(stage)) = self.stages.remove(&stage_id) {
    ...
    fn rollback_resolved_stage(&mut self, stage_id: usize) -> scorpio_core::error::Result<bool> {
    ...
    fn succeed_job(&mut self) -> scorpio_core::error::Result<()> {
        ...
        let partition_location = self
            ...
            .into_iter()
            .map(|l| l.try_into())
            .collect::<scorpio_core::error::Result<Vec<_>>>()?;
    ...
    fn pop_next_task(
        &mut self,
        executor_id: &str,
    ) -> scorpio_core::error::Result<Option<super::execution_graph::TaskDescription>> {
        if matches!(
            self.status,
            JobStatus {
                status: Some(job_status::Status::Failed(_)),
        ...
        let mut next_task = self.stages.iter_mut().find(|(_stage_id, stage)| {
            ...
            if let ExecutionStage::Running(stage) = stage {
                ...
                let task_info = crate::state::execution_graph::TaskInfo {
                    ...
                    end_exec_time: 0,
                    finish_time: 0,
                    task_status: task_status::Status::Running(scorpio_core::serde::protobuf::RunningTask {
