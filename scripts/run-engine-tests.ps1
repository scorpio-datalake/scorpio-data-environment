...
.DESCRIPTION
  Defaults to the scorpio-core package so a small change does not compile the whole workspace.
...
.PARAMETER BuildBinary
  With a single package (not -Workspace), pass --features build-binary. Required for scorpio-core tests
  in the object_store module (S3/GCS/Azure registry). Ignored when -Workspace is set.

.PARAMETER Package
  Crate name (-p), default scorpio-core. Ignored when -Workspace is set.
