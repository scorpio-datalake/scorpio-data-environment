...
.SYNOPSIS
  ...
  If `scorpio-core` is missing from the workspace, prints what to do instead.
#>
...
try {
    ...
    if (-not $hasCore) {
        ...
        Write-Host "Add `"scorpio/core`" (package scorpio-core) to [workspace].members in engine/Cargo.toml if needed, or run full workspace tests:" -ForegroundColor Yellow
